...
test('getRemainingTicketCount uses event capacity minus issued tickets when capacity is present', async () => {
...
test('getRemainingTicketCount falls back to Square inventory when capacity is missing', async () => {
  const remaining = await getRemainingTicketCount(
    { id: 'event_1', square_variation_id: 'var_1' },
    {
      getVariationInventory: async () => ({ available: 7 }),
      countTicketsByEventId: async () => 49,
    },
...
test('sendDiscordTicketNotification posts buyer name and remaining tickets through the Discord bot API', async () => {
  ...
  const result = await sendDiscordTicketNotification(
    ...
    { id: 'event_5', name: 'Launch', capacity: 36, square_variation_id: 'var_launch' },
    'Ada',
    {
      getBaseConfig: () => ({
  ...
  assert.match(JSON.parse(requests[0].options.body).content, /Ada bought a ticket for Launch\. 34 tickets left\./);
  assert.equal(result.remainingTickets, 34);
});
