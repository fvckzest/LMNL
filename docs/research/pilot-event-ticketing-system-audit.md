...
It is **not yet a reliable LMNL Managed Ticketing Pilot rail**. The pilot's hard requirements have no durable model or operational controls for seller/recipient designation, all-in-price and policy acceptance, fixed-capacity enforcement, cancellation/refund, chargebacks, tax, Square fee evidence, 10% LMNL Fee, Recipient Share, manual Recipient Settlement, or an event-level reconciliation/audit packet. The legal-and-tax research adds an independent launch gate: do not sell until Square, Washington counsel, and a CPA confirm the seller/payment/settlement structure. See [the legal and tax research in PR #14](https://github.com/fvckzest/LMNL/pull/14).
...
| Buyer disclosures and records | **Gap** | There is no purchase-time disclosure/version/acceptance record for seller identity, all-in price/tax, final-sale policy, cancellation refund, or event terms. |
...
| Reconciliation and settlement | **Gap** | No financial tables/services for payments, fees, taxes, refunds, disputes, fee/share calculation, payment proof, or settlement approval. |
...
- There is no first-class Settlement Recipient, LMNL Fee, Recipient Share, agreement reference, seller-of-record, tax treatment, or policy version field in the form/repository shape ([`api/_lib/repositories/events.js:84-125`](../../api/_lib/repositories/events.js#L84-L125)). Metadata is technically extensible, but it is not a reviewed financial/audit model.
...
- No source applies a ticket tax, records whether tax is included, or removes tax from the settlement base. This directly conflicts with the pilot research requirement to classify and separately account for tax if applicable.
...
- Current operational reporting is ticket counts/activity rather than payment truth: `getSpaceTicketActivity` counts ticket rows, not completed payment amounts, fees, tax, refunds, or disputes ([`api/_lib/services/space-activity.js:45-75`](../../api/_lib/services/space-activity.js#L45-L75)).
...
- Source searches found no buyer-facing policy for all-in price, tax inclusion/exclusion, seller contact, final sale/no buyer refunds, cancellation trigger/full refund, age/accessibility/entry conditions, or affirmative pre-purchase acceptance/version capture. The hosted checkout configuration contains neither policy copy nor a record of acceptance ([`api/_lib/services/checkout.js:360-365`](../../api/_lib/services/checkout.js#L360-L365)).
- No refund/cancellation/chargeback/dispute/settlement/reconciliation/tax entity or Square refund/dispute call appears in `api/`, `src/`, `sql/`, or tests. Existing event status has no cancellation lifecycle. This is a source-level absence, not proof that no manual Square action has ever been taken.
- Consequently there is no event-level record of completed payments, Square fees, tax, refunds, disputes/chargebacks, the 10% LMNL Fee, Recipient Share, manual settlement due date/payment reference, post-settlement chargeback recovery, approver, or signed Pilot Event Agreement. These must be modeled before a ticket can be responsibly reconciled against Square's gross reporting and the pilot's five-business-day settlement promise.

## Required work before Pilot Event sales open

1. **Clear the external no-sales gates first.** Obtain the Square seller-structure answer, Washington counsel's confirmation, CPA/tax classification, registrations, and written Pilot Event Agreement described in the legal/tax memo. System work cannot make an impermissible processor arrangement valid.
2. **Create an immutable Pilot Event configuration and sales lifecycle.** Include disclosed LMNL seller/contact, Settlement Recipient reference, agreement/version, all-in price and tax treatment, fixed capacity, date/venue, fixed 10% fee, and sales-open/cancelled states. Prevent material edits after sales open except a documented cancellation flow.
...
4. **Add buyer terms and evidence.** Display and record the exact policy/version accepted before payment: LMNL seller/contact, all-in amount and tax treatment, final-sale rule, full refund only on Pilot Event Cancellation, date/time/address, capacity/GA/access constraints, and cancellation communication path.
...
6. **Build the financial record and reconciliation packet.** Import or record Square payment/refund/dispute/fee evidence; separately record taxes; calculate `Net Ticket Revenue = completed ticket payments − refunds − chargebacks − payment-processing fees` only after excluding/remitting taxes where applicable; calculate the fixed 10% LMNL Fee and Recipient Share; retain reviewer, payment method/date/reference, and recovery balance.
...
The repository contains only migration scripts for certain tables, not a full current database schema dump, and the current production configuration was not queried. Confirm applied SQL/RLS, webhook delivery, Square inventory behavior, credentials, and deployed buyer flow during the readiness work. The legal/tax conclusions in the linked memo remain subject to Washington counsel, CPA, and Square confirmation.
