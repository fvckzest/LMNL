...
| Issuance, ticket delivery, wallet | **Partial** | Webhook fulfillment can issue a QR ticket, email, optional Apple Wallet pass, and protect duplicate ticket rows per Square order, but signature verification fails open when its secret or the incoming signature is absent. Email/pass failures are also not durably queued for recovery. |
| Check-in | **Partial** | An authenticated LMNL Admin can validate and mark a QR ticket used, but the update is not atomic and concurrent scans can both admit the same unused ticket. |
...
- The Square webhook calculates the request URL, passes the raw body to signature verification, and dispatches to fulfillment ([`api/[...route].js:1189-1200`](../../api/%5B...route%5D.js#L1189-L1200), [`api/_lib/services/webhook-fulfillment.js:86-113`](../../api/_lib/services/webhook-fulfillment.js#L86-L113)). However, verification returns without rejecting when either the configured signature key or incoming signature is absent. Production fulfillment must fail closed before the pilot opens sales.
...
- `confirmCheckInTicket` reports tickets already marked used, updates `is_used`/`used_at`, and attempts to record attendance verification ([`api/_lib/services/tickets.js:66-120`](../../api/_lib/services/tickets.js#L66-L120), [`api/_lib/repositories/tickets.js:224-238`](../../api/_lib/repositories/tickets.js#L224-L238)). The read and update are separate, and the update filters only by ticket ID; two concurrent scans can both read unused and both return `checked_in`. The pilot needs an atomic unused-to-used claim while retaining the LMNL-Admin-only operating model.
...
1. **Complete the map's narrow external gates first.** This source audit does not decide them; follow the separate Pilot Event boundary work for Square's seller-structure answer, the event-specific agreement review, and the single event-level tax setting. This casual pilot does not require a tax system or broad marketplace analysis.
...
7. **Harden operational recovery, admission, and privacy.** Fail closed when the Square webhook secret or signature is missing; persist webhook receipt/outcome and a retryable fulfillment job; make check-in an atomic unused-to-used claim; supply an admin reconciliation/retry screen; use immutable event IDs for matching; verify buyer ticket access controls; test the deployed webhook, email, Wallet configuration, and migration state with a nonproduction transaction.
